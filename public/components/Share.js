class Share extends AbstractCloudFunctionClient {
  constructor(props) {
    ...
    this.state.processingMsg =
      <i className="fa fa-gear w3-xxlarge w3-spin"></i>;
    ...
    return (
      <div className="share">
          ...
          <div className="w3-modal-content link-modal">
            <header className="w3-container datasheet-header">
              {this.props.lang == 'en' ?
                ...
                'Lien vers les paramètres actuels'
              }:
            </header>
            <span
              className="w3-btn w3-display-topright close"
              onClick={(event) => this.setState({state: 'idle'})}>
        ...
        <button
          ...
          &nbsp;
          {this.props.lang == 'en' ? 'Share Profiles': 'Partager ces Profils'}
        </button>
      </div>
    );
  }
